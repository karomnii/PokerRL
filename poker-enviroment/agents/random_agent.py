...
class RandomAgent(IAgent):
    def act(self, observation: dict) -> Tuple[Action, Optional[int]]:
        ...
        if rand_value < 0.05:
            ...
            if observation.get("call_amount",200) > observation.get("chips", 1000)//3 and random.random() < 0.5:
                return (Action.FOLD, None)
            ...
            min_raise = observation.get("call_amount", 20) if observation.get("call_amount", 20) > 0 else 20
